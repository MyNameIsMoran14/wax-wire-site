...
// Ported from reactbits.dev/backgrounds/gradient-blinds (MIT-style, free to use).
// WebGL shader via `ogl` — kept verbatim, only typed for our project.
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  vec2 uv0Aspect = uv0; uv0Aspect.x *= aspect;
  vec2 offsetAspect = offset; offsetAspect.x *= aspect;
  float d = length(uv0Aspect - offsetAspect);
...
`
    ...
    const loop = (t: number) => {
      ...
      // The spotlight starts centered and eases toward wherever the mouse
      // actually is — on a fresh mount/reload that catch-up reads as a
      // jarring jump. Fading the spot in from 0 opacity over the same
      // window hides the repositioning; only the settled position is seen.
      if (entryStart === null) entryStart = t
      const entryT = Math.min(1, (t - entryStart) / ENTRY_FADE_MS)
      uniforms.uSpotlightOpacity.value = spotlightOpacity * entryT
      if (mouseDampening > 0) {
  ...
  return (
    <div
      ...
      style={!lightMode && mixBlendMode ? { mixBlendMode } : undefined}
    />
  )
}
