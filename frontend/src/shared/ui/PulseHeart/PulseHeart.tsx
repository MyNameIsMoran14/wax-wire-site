...
// Ported from reactbits.dev/micro/pulse-heart — a heart toggle with a
// squash-and-stretch "pulse" on click plus a rolling-digit counter. Logic is
// untouched from the source; only typed for TS and re-pointed at our own
// brand colors (see PulseHeart.css) by default.
export function PulseHeart({
  ...
  const liked = controlled ? (likedProp as boolean) : inner
  ...
  const startRoll = (from: number, to: number) => {
    ...
    clearTimeout(rollTimer.current)
    rollTimer.current = window.setTimeout(() => setRoll(null), cfg.current.rollDuration)
  }
  ...
  const cells: ({ ch: string } | { top: string; bottom: string })[] = roll
    ? roll.at === -1
      ? [{ top: roll.up ? roll.a : roll.b, bottom: roll.up ? roll.b : roll.a }]
      : [...roll.b].map((ch, i) => (i === roll.at ? { top: roll.up ? roll.a[i] : ch, bottom: roll.up ? ch : roll.a[i] } : { ch }))
    : [...text].map((ch) => ({ ch }))
      ...
      <span ref={pillRef} className="pulse-heart__pill">
        <span ref={heartRef} className="pulse-heart__heart" aria-hidden="true">
          <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <g ref={glyphRef}>
              {FavouriteIcon.map(([, attrs], i) => (
                <path key={i} d={String(attrs.d)} vectorEffect="non-scaling-stroke" />
              ))}
            </g>
          </svg>
        </span>
