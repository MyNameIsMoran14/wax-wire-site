...
// Ported from reactbits.dev/micro/flip-card — 3D tilt/drag/glare flip card
// built on `motion`. Logic untouched from the source; only typed for TS and
// re-pointed at our own brand colors by default (see FlipCard.css).
export default function FlipCard({
  ...
  const shown = controlled ? (flipped as boolean) : inner
  ...
  const settle = (to: number, velocity: number, instant: boolean | undefined) => {
    spin.current?.stop()
    target.current = to
  ...
  const onPointerDown = (e: ReactPointerEvent<HTMLDivElement>) => {
    ...
    try {
      ...
      // Pointer capture can fail silently on some platforms — non-fatal.
    }
    spin.current?.stop()
    grip.current = {
      id: e.pointerId,
  ...
  const release = (e: ReactPointerEvent<HTMLDivElement>, cancelled: boolean) => {
    ...
    try {
      ...
      // Same as above — releasing an already-lost capture is harmless.
    }
    setDragging(false)
